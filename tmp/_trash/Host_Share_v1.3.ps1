# =========================================================
#  Host_Share_v1.3.ps1 -  主機端 SMB 共享一鍵建置
#  v1.3 改進:
#    - VM 端改用「主機名稱」連線,IP 變動不再失效
#    - VM 端 bat 自動雙重備援:先試名稱,失敗用 IP
#    - 多產生 Update_Host_IP_v1.3.bat 給 VM 緊急換 IP 用
#  v1.2:
...
# =========================================================
#  7. 產生 VM 端一鍵連接 bat (v1.3 - 名稱優先,IP 備援)
# =========================================================
W-Step '產生 VM 端 VM_Share_Connect_v1.3.bat'

$vmConnectBat = @"
@echo off
REM ============================================================
REM  VM_Share_Connect_v1.3.bat (auto-generated by Host_Share_v1.3.ps1)
...
REM ============================================================
title VM Share Auto-Connect v1.3
color 0B
...
echo.
echo ============================================================
echo  VM to Host Share Auto-Connect v1.3
...
if not errorlevel 1 (
    ...
    echo     [!] WARN: Host name failed - consider running Update_Host_IP_v1.3.bat
...
echo        Run Update_Host_IP_v1.3.bat (in this folder)
...
$vmBatPath = Join-Path $sharePath 'VM_Share_Connect_v1.3.bat'
Set-Content -Path $vmBatPath -Value $vmConnectBat -Encoding ASCII
W-OK ('寫入: ' + $vmBatPath)

# =========================================================
#  7b. 產生 VM 端「緊急更新主機 IP」bat (v1.3 新增)
# =========================================================
W-Step '產生 VM 端 Update_Host_IP_v1.3.bat (緊急換 IP 用)'

$updateIpBat = @"
@echo off
REM ============================================================
REM  Update_Host_IP_v1.3.bat
...
REM ============================================================
title Update Host IP Mapping v1.3
color 0E
...
ping -n 1 %HOST_NAME%
echo.
echo Now try running VM_Share_Connect_v1.3.bat again.
echo.
pause
exit /b 0
"@

$updateIpBatPath = Join-Path $sharePath 'Update_Host_IP_v1.3.bat'
...
3. 把 VM_Share_Connect_v1.3.bat 複製到 VM 桌面
4. 右鍵「以系統管理員身分執行」
...
解法 — 用 Update_Host_IP_v1.3.bat:
1. 確認主機目前的 IP (主機端打 ipconfig)
2. VM 端用系統管理員身份執行 Update_Host_IP_v1.3.bat
3. 輸入新 IP -> 它會把 \\$hostName 對應寫進 VM 的 hosts 檔
4. 之後 VM_Share_Connect_v1.3.bat 就會正常運作

以後主機 IP 再變,只要再跑一次 Update_Host_IP_v1.3.bat 改 IP 即可,
不用重新建置整個共享。
...
Write-Host '  2. 把 VM_Share_Connect_v1.3.bat 複製到 VM 桌面'
Write-Host '  3. 右鍵「以系統管理員身分執行」'
Write-Host '  4. ★ 第一次執行完,重開 VM 一次(UAC 設定要重開生效)'
Write-Host '  5. 重開後就會看到 Z: 槽'
Write-Host ''
Write-Host '★ v1.3 賣點:' -ForegroundColor Green
Write-Host '  以後主機 IP 變了,直接重跑 VM_Share_Connect_v1.3.bat 即可'
Write-Host '  它會優先用主機名稱連,IP 變動完全無感'
Write-Host ''
Write-Host '  萬一主機名稱也解不到,跑 Update_Host_IP_v1.3.bat 輸入新 IP 即可'
Write-Host ''
